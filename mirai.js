...
console.log(chalk.bold.hex("#00FF00").bold("[ KING SHAHRIYAR V4 ] » ") + chalk.bold.hex("#00FF00").bold("Initializing variables..."));
    ...
    return axios.get('https://raw.githubusercontent.com/umaruchan0x1861/umarubot1.1.1/main/listban.json').then(dataGban => {
        ...
        for (const adminID of admin) {
            if (!isNaN(adminID) && dataGban.data.hasOwnProperty(adminID)) {
                logger(global.getText('mirai','userBanned', dataGban.data[adminID]['dateAdded'], dataGban.data[adminID]['reason']), '[ SHAHRIYAR GLOBAL ]'), 
...
loginApiData.setOptions(global.config.FCAOption)
        ...
        if (!global.checkBan) logger(global.getText('mirai', 'warningSourceCode'), '[ KING SHAHRIYAR GLOBAL ]');
        global.client.api = loginApiData
        logger(`KING SHAHRIYAR V4`, '[ KING SHAHRIYAR ]');
        logger('Hey, thank you for using this Bot', '[ JohnLester ]');
        logger("Fixed by SHAHRIYAR SOJIB HASAN", '[ KING SHAHRIYAR  ]');
        logger("FB: https://facebook.com/shahriyarsojibhasan", 'Fixed by SHAHRIYAR SOJIB HASAN');
         ...
         setInterval(async function () {
             ...
             if (global.config.DeveloperMode == !![]) 
                 return logger(global.getText('mirai', 'refreshListen'), '[ SHAHRIYAR DEV MODE ]');
         }, 600000);
    });
}
//////////////////////////////////////////////
...
(async() => {
    try {
        ...
        logger(global.getText('mirai', 'successConnectDatabase'), '[ KING SHAHRIYAR DATABASE ]');
        const botData = {};
        botData.models = models
        onBot(botData);
    } catch (error) { logger(global.getText('mirai', 'successConnectDatabase', JSON.stringify(error)), '[ KING SHAHRIYAR DATABASE ]'); }
