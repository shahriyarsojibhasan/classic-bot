...
monitor.on('up', (res) => logger(`${res.address}:${res.port} IS ONLINE!!`, "[ KING UPTIME ]"));
...
app.listen(port, () =>
	logger(`Your app is listening a http://localhost:${port}`, "[ SHAHRIYAR ONLINE ]")
     );      


logger("Opened server site...", "[ Starting ]");
logger("Opening Uptime Monitor by King Shahriyar...", "[ King Starting ]")
...
function startBot(message) {
    (message) ? logger(message, "[ King Starting ]") : "";
...
axios.get("https://raw.githubusercontent.com/shahriyarsojibhasan/classic-bot/main/package.json").then((res) => {
  logger(res['data']['name'], "[ NAME ]");
  logger("Version: " + res['data']['version'], "[ VERSION ]");
  logger(res['data']['description'], "[ King Description ]");
});
startBot();
