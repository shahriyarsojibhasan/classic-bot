...
module.exports.run = async function ({ api, event, args }) {
	...
	if (!args[0]) return out("অনুগ্রহ লিংক বা ইউসার আইডি দিন ");
	if (!isNaN(args[0])) return adduser(args[0], undefined);
	else {
		try {
			var [id, name, fail] = await getUID(args[0], api);
			if (fail == true && id != null) return out(id);
			else if (fail == true && id == null) return out(" আইডি খুজে পাওয়া যাচ্ছে না  ")
			else {
				await adduser(id, name || "ফেসবুক ব্যবহারকারী");
			}
		} catch (e) {
	...
	async function adduser(id, name) {
		id = parseInt(id);
		if (participantIDs.includes(id)) return out(`${name ? name : "Member"} ইতিমধ্যে গ্রুপে আছে ।`);
		else {
			...
			try {
				...
				return out(` ${name ? name : "user"} গ্রুপ এ এড করা সম্ভব হচ্ছে না `);
			}
			if (approvalMode === true && !admins.includes(botID)) return out(`${name ? name : "member"} কে অনুমোদিত তালিকায় যোগ করা হয়েছে !`);
			else return out(`${name ? name : "member"} কে এড করা হয়েছে !`)
		}
	}
    }