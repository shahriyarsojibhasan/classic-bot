...
module.exports.handleReply = async function({ api, event, handleReply }) {
	...
	api.sendMessage(`💓──── •🎶• ────💓\n-------------------------\n${body}\nআপনার গান টি প্রস্তুত করা হচ্ছে ✨\n-------------------------\n💓──── •🎶• ────💓`, event.threadID, (err, info) =>
	...
	try {
		...
		ytdl(handleReply.link[event.body - 1])
			...
			.on("close", () => {
				if (statSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.m4a`).size > 26214400) return api.sendMessage('❗আপনার গানটি পাঠানো সম্ভব হচ্ছে না । এটা 25MB এর বড় । আপনি /music ব্যাবহার করতে পারেন ।  .', event.threadID, () => unlinkSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.m4a`), event.messageID);
...
let getthumnail = (await axios.get(`${linkthumnail}`, { responseType: 'arraybuffer' })).data;
  ...
  imgthumnail.push(fs.createReadStream(__dirname + `/cache/${numb}.png`));
      ...
      var body = `»🔎 【${link.length}】টি অনুসন্ধান ফলাফল । শাহরিয়ার সজিব হাসান  \n\n🍑──── •🎶• ────🍑\n${msg}»আপনার পছন্দের গানের নম্বর রিপ্লাই করেন `
...
let datab = (await axios.get(`https://www.googleapis.com/youtube/v3/videos?part=contentDetails&id=${value.id}&key=${keyapi}`)).data;
  ...
  imgthumnail.push(fs.createReadStream(__dirname + `/cache/${numb}.png`));
      ...
      var body = `»🔎  ${link.length}】টি অনুসন্ধান ফলালল । SSH BOT  \n\n🍑──── •🎶• ────🍑\n${msg}» পছন্দ অনুযায়ী গানের নম্বর রিপ্লাই দিন `
return api.sendMessage({attachment: imgthumnail, body: body}, event.threadID,(error, info) => global.client.handleReply.push({ 
  name: this.config.name, 
  messageID: info.messageID, 
  author: event.senderID, 
